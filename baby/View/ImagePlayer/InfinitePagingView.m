...
- (void)scrollToRowAtIndex:(int)index animated:(BOOL)animated {
    int maxPages = [self.delegate numberOfPages];
...
- (void)tilePages {
    ...
    int firstNeededPageIndex = floorf(CGRectGetMinX(visibleBounds) / [self.delegate widthForPage]);
    int lastNeededPageIndex = floorf((CGRectGetMaxX(visibleBounds)-1) / [self.delegate widthForPage]);
