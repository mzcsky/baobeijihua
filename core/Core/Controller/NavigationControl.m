...
- (void)popToIndex:(int)index animation:(ViewSwitchAnimation)animation {
    ...
    for (int i = index + 1; i < viewControllerStack.count - 1; i++) {
...
- (void)popToViewController:(id)ctr animation:(ViewSwitchAnimation)animation {
    if (stackLock)
        return;
    int index = [viewControllerStack indexOfObject:ctr];
    if (index != NSNotFound) {
